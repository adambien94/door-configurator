export default {
  message: {
    loginTitle: "Zaloguj się",
    email: "Adres email",
    password: "Hasło",
    remember: "Zapamiętaj mnie",
    login: "Zaloguj",
    processing: "Przetwarzanie",
    myOrganization: "Moja organizacja",
    step: "krok",
    chooseDoor: "Wybierz drzwi",
    chooseDivision: "Wybierz podział drzwi",
    chooseColor: "Wybierz kolor",
    doorType: "Typ drzwi",
    singleDoor: "Pojedyńcze",
    doubleDoor: "Podwójne",
    tripleDoor: "Potrójne",
    doorSize: "Rozmiar drzwi",
    width: "Szerokość",
    height: "Wysokość",
    doorDivision: "Podział drzwi",
    numberOfBeams: "Liczba belek",
    numberOfPosts: "Liczba słupków",
    thickness: "Grubość",
    share: "udostępnij",
    back: "wstecz",
    nextStep: "dalej",
    name: "Nazwa",
    phoneNumber: "Numer telefonu",
    address: "Adres",
    city: "Miejscowość",
    postalCode: "Kod pocztowy",
    loginError: "Niepoprawny email lub hasło ( próbuj dalej ) 😀",
    link: "Link",
    copiedToClipboard: "skopiowany do schowka",
    selectLanguage: "Wybierz język"
  },
  colors: {
    black: "czerń",
    gray: "grafit",
    white: "biel",
    blue: "błękit",
    green: "zieleń",
    gold: "złoto",
    coral: "łosoś"
  },
  languages: {
    pl: "Polski",
    en: "Angielski"
  }
};

// {
//   "loginTitle": "Zaloguj się",
//   "email": "Adres email",
//   "password": "Hasło",
//   "remember": "Zapamiętaj mnie",
//   "login": "Zaloguj",
//   "processing": "Przetwarzanie",
//   "myOrganization": "Moja organizacja",
//   "step": "krok",
//   "chooseDoor": "Wybierz drzwi",
//   "chooseDivision": "Wybierz podział drzwi",
//   "chooseColor": "Wybierz kolor",
//   "doorType": "Typ drzwi",
//   "singleDoor": "Pojedyńcze",
//   "doubleDoor": "Podwójne",
//   "doorSize": "Rozmiar drzwi",
//   "width": "Szerokość",
//   "height": "Wysokość",
//   "doorDivision": "Podział drzwi",
//   "numberOfBeams": "Liczba belek",
//   "numberOfPosts": "Liczba słupków",
//   "share": "udostępnij",
//   "back": "wstecz",
//   "nextStep": "dalej",
//   "name": "Nazwa",
//   "phoneNumber": "Numer telefonu",
//   "address": "Adres",
//   "city": "Miejscowość",
//   "postalCode": "Kod pocztowy",
//   "loginError": "Niepoprawny email lub hasło",
//   "link": "Link",
//   "copiedToClipboard": "skopiowany do schowka",
//   "selectLanguage": "Wybierz język",

//   "colors": {
//     "black": "Czarny",
//     "white": "Biały",

//     "gray": "Szary",

//     "neon": "Zielony",
//     "sky": "Błękitny"
//   },
//   "languages": {
//     "en": "Angielski",
//     "pl": "Polski"
//   }
// }
